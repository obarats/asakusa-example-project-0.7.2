...
import com.example.modelgen.dmdl.io.CategorySummaryInput;
import com.example.modelgen.dmdl.io.CategorySummaryOutput;
 ...
 */
@DataModelKind("DMDL")@ModelInputLocation(CategorySummaryInput.class)@ModelOutputLocation(CategorySummaryOutput.class)@
        PropertyOrder({"category_code", "amount_total", "selling_price_total"})@Summarized(term = @Summarized.Term(
        source = JoinedSalesInfo.class, foldings = {@Summarized.Folding(aggregator = Summarized.Aggregator.ANY, source = 
            "categoryCode", destination = "categoryCode"),@Summarized.Folding(aggregator = Summarized.Aggregator.SUM, 
            source = "amount", destination = "amountTotal"),@Summarized.Folding(aggregator = Summarized.Aggregator.SUM, 
            source = "sellingPrice", destination = "sellingPriceTotal")}, shuffle = @Key(group = {"categoryCode"}))) 
        public class CategorySummary implements DataModel<CategorySummary>, Writable {
